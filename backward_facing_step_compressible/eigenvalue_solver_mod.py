...
def setupGeneralizedNonHermitianEPS(LEP, shift):
    ...
    LEP.setDimensions(60, PETSc.DEFAULT, PETSc.DEFAULT)
    LEP.setTarget(shift)
...
setupGeneralizedNonHermitianEPS(LEP, shift = -0.07*(2*3.415)*1j)#0.7*1j) #lambda=6+2pi*St
...
if nconv > 0:
    ...
    # Plot spectrum.
    plt.xlim([0.025, 0.2])
    plt.scatter(-np.imag(spectrum)/(2*np.pi), -np.real(spectrum))
    plt.ylabel("growth rate")
    plt.xlabel("St")
    plt.savefig(case_path + "spectrum.png")
